...
| A17 | 2026-07-21 | Content-detail previews are an intentional extension of A14: extract **only the visible requested `ContentItem` page**, on demand through the indexer worker, cache by `ContentItemId`, and fall back to the representative package preview. | Enables investigation without a whole-library image sweep or competing GUI writes. Entry-size, pixel and decompression caps remain mandatory; non-requested content images remain deferred. |
| A18 | 2026-07-21 | Library bulk selection is **package-id based** over the OrderedSnapshot (select-all-matching), with loaded-row projection for DataGrid checkboxes. Gallery uses a multi-column `WrapPanel` over the windowed loaded cards; full TreeDataGrid/O(1) scrollbar awaits Avalonia 12 + licensing. | Clears select-all/gallery-wall debt without a UI rewrite; A9 ID backbone + `GetByIdsAsync` already avoid deep OFFSET for LoadMore. |
