...
- [x] **DLG-7 · m-preset (edit).** *BE:* `IPresetService`. *Done:* `PresetEditViewModel(IPresetService)` + `Views/PresetEditDialog.axaml` (name, add-member, activation preview). T: `PresetEditDialogTests.Add_member_persists_and_previews`.
