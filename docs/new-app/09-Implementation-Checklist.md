...
- [x] 0.25 Single-writer queue: catalog mutations serialize through `IWriteQueue` inside stream ingest / worker · T: `ScalableIndexingArchitectureTests.Stream_indexer_indexes_a_var_and_sets_raw_stored` + `Indexer_worker_coalesces_duplicate_starts` — `StreamIndexer` enqueues all mutations; `IndexerWorker` coalesces starts (A12)
- [x] 0.26 Dirty-set mechanism: durable `DirtyPackage` rows · T: `ScalableIndexingArchitectureTests.Durable_dirty_set_survives_mark_and_drain` — `DurableDirtySet` / `DirtyPackage` table (A15)
...
- [x] 1.8 `MinFreeBytes` reserve stored and honored by placement · T: `PlacementCapacityTests` (room only when free−minFree ≥ size; unknown free → no room) — `PlacementCapacity.HasRoom` + `FreeSpaceLedger` (concurrent reservations honor MinFree, 5.10) — *field stored; placement engine pending*
...
- [x] 1.23 Staged raw-first: stream ingest → `RawStored` (+ optional representative thumb in one-handle) → chunked read-model refresh; deps resolve as later phase · T: `ScalableIndexingArchitectureTests.Stream_indexer_indexes_a_var_and_sets_raw_stored` + `One_handle_inspector_extracts_meta_without_unbounded_read` + `Orchestrator_uses_stream_path_end_to_end` (A13/A14)
- [x] 1.24 Per-physical-drive parallelism: HDD degree=1, NVMe higher · T: `ParallelismPolicyTests` + `StreamIndexer` uses `IngestLimits.DegreeFor(mediaType)` / `ParallelismPolicy.DegreeFor` (A12)
- [x] 1.25 Bulk writes via write queue; WAL checkpoint + `ANALYZE` after ingest; pragmas on every connection · T: `StreamIndexer` calls `CheckpointWalAsync`/`AnalyzeAsync`; `SqlitePragmaInterceptor` + `CatalogDatabaseInitializer` in `Bootstrap.BuildApp` · `ScalableIndexingArchitectureTests`
...
- [x] 1.41 Faceted count debounced + approximate during typing, exact on settle · T: `LibraryViewModelStateTests.Typing_flags_count_approximate_then_exact_after_settle` + `Rapid_typing_coalesces_into_a_single_refresh` — `LibraryViewModel` debounces search (250 ms, injectable delay), flags `IsCountApproximate` until settle; keystrokes coalesce (superseded refreshes cancelled → one query). View shows a "~" prefix. (`MainWindow.axaml`)
...
- [x] 2.6 Per-`(Creator,Package)` "current latest" deref, index-backed O(1) · T: `ForwardClosureFlowTests.Current_latest_returns_the_highest_version` (v1/v2/v3 → latest=v3; unknown family → null) — `EfDependencyGraph.CurrentLatestAsync` (backed by `IX_Package_Creator_PackageName_VersionSort`) — *resolver builds an in-memory family map (latest via MaxBy); a persisted O(1) pointer is a later optimization*
- [x] 2.7 Incremental / paged re-resolve: `ResolveAllAsync` and `ResolveFamilyAsync` page dependencies (no full-catalog `ToList` of edges+packages tracked) · T: `ScalableIndexingArchitectureTests.Orchestrator_uses_stream_path_end_to_end` + existing family test — `EfDependencyResolver` keyset pages (A13)
...
- [x] 2.14 Missing-deps screen lists refs with needed-by counts · T: `DependencyResolutionFlowTests.Missing_deps_query_lists_refs_with_needed_by_counts` (Ghost.Common.1 needed-by=2, most-needed first) + `MissingDepsViewModelTests` — `IMissingDepsQuery`/`EfMissingDepsQuery` + `MissingDepsViewModel` (*scan variants filter the same query*)
...
- [x] 3.7 Activate preset = resolve members → forward closure → create links; missing reported · T: `ActivationFlowTests.Builds_links_for_members_and_their_closure` (2 links from 1 member + its dep) + `Rebuild_replaces_prior_links` — `EfActivationService.BuildProfileLinksAsync` (*symlink files created by `SymlinkService` where privilege allows; alias application 3.9 pending*)
...
- [x] 3.9 🔒 Persistent aliases (global + per-preset) re-apply automatically on every build — no re-setup · T: `ActivationFlowTests.Persistent_alias_reapplies_on_every_build` (a missing member resolved via a global `VarAlias`; a rebuild re-applies it) — `EfActivationService.ResolvedMemberIdsAsync`
...
- [x] 5.9 🔒⚠ `synchronous=FULL` on any transaction gating a destructive FS op · T: `SynchronousPragmaTests.Full_and_normal_synchronous_round_trip` (NORMAL=1→FULL=2→NORMAL=1) — `SqlitePragmas.ApplySynchronousFull`, applied around the source-delete in `MigrationRunner`
...
- [x] X.2 ⚠ Trash quota + per-item restore manifest; restore works with DB absent · T: `FileTrashServiceTests.Restore_works_from_the_manifest_alone_without_a_database` + `List_enumerates_trashed_items_from_manifests` + `Quota_evicts_the_oldest_trashed_items` (bounded trash, oldest aged out — never a silent hard-delete of the source)
...
- [x] X.9 Import-from-old-varManager: recognize quarantine dirs + ingest `.fav`/`.hide` sidecars → ContentItemPref · T: `VarManagerImportTests` (parse `.fav`/`.hide`/`.FAV` → content name + `ContentItemPrefState`; reject non-sidecars; recognize `___VarRedundant____*`/`___VarnotComplyRule___` but **not** `___AddonPacksSwitch ___`; `Scan_recovers_prefs_and_reports_quarantine_dirs` — sidecars under a quarantine dir excluded) — `VarManagerImport` (pure parse + directory walk → `ImportScanResult{Prefs, QuarantineDirectories}`; quarantine names match the real test corpus)
