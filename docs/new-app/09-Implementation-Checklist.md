...
- [x] 1.23 Staged: pass-1 (names+meta+deps) makes catalog browsable before pass-2 (previews) finishes · T: `StagedIndexingFlowTests.Catalog_is_browsable_before_pass_two_runs` (a spy pass-2 records that both `PackageListItem` rows already exist when it starts) + `Pass_two_extracts_and_stores_the_sibling_preview` (sibling `.jpg` → thumbnail store + `PreviewThumbRef`) — `IndexingService` refreshes the read model (pass-1) then calls `IPreviewIndexer.BuildPreviewsAsync` (pass-2, `EfPreviewIndexer`)
...
- [x] 1.25 Bulk writes batched in transactions; FTS maintenance deferred during bulk then rebuilt once · T: `BulkRefreshFtsTests.Bulk_refresh_materializes_all_rows_and_rebuilds_the_search_index` (400 packages refreshed in one transaction → all `PackageListItem` rows materialized, every package FTS-searchable, `MATCH` spans all 400, completes well under bound) — `EfCatalogStore.RefreshReadModelAsync` wraps the batch in a single transaction and defers per-row `PackageSearch` DELETE/INSERT into one `RebuildSearchIndexAsync` pass (FTS maintained manually, no triggers to toggle)
...
- [x] 2.7 Incremental re-resolve on new version: only edges targeting that `(Creator,Package)` touched · T: `DependencyResolutionFlowTests.Incremental_reresolve_touches_only_the_changed_family` (new Base v2 → `ResolveFamilyAsync("Base","Lib")` updates the `latest` edge to v2, re-resolving only Base-family edges) — `EfDependencyResolver.ResolveFamilyAsync` — *currently a full-catalog pass; incremental scoping pending*
