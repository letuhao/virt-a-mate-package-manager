    ...
    IIndexerClient indexer) : IOnboardingService
{
    public async Task<Result<OnboardingResult>> AddAndIndexAsync(string name, string path, long? reserveBytes = null, CancellationToken cancellationToken = default)
    {
        ...
        var start = await indexer.StartIndexRepositoryAsync(registered.Value.Id, forceFull: true, cancellationToken: cancellationToken).ConfigureAwait(false);
        if (start.IsFailure)
            return Result.Failure<OnboardingResult>(start.Error.Code, start.Error.Message);

        try
        {
            await IndexerAwait.WaitForJobAsync(indexer, start.Value, progress: null, cancellationToken).ConfigureAwait(false);
        }
        ...
        {
            return Result.Failure<OnboardingResult>("indexer.failed", ex.Message);
        }

        var last = (await indexer.GetStatusAsync(cancellationToken).ConfigureAwait(false)).Value;
